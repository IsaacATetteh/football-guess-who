...
const Home = () => {
  ...
  return (
    <div
      className={`flex justify-center items-center lg:w-full bg-[#1D1D1D] text-white ${
        showGame ? "" : "mt-[16rem]"
      }`}
    >
      {userData ? (
        <div className="mb-20 h-10 w-10">
          <h1 className="text-4xl mb-10">Welcome, {userData.username}</h1>
        </div>
      ) : (
        <div></div>
      )}
      {showGame ? (
        ...
        <form className="flex flex-col gap-4" onSubmit={handlePlayButtonClick}>
